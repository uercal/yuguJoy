...
class Goods extends BaseModel
{
    ...
    {
        $category_ids = Category::where('is_index', '=', 10)->column('category_id');

        $data = $this->with(['spec', 'category', 'image.file'])
            ->where('is_delete', '=', 0)
            ->where('goods_status', '=', 10)
        ...
        foreach ($data as $key => $value) {            
            if(isset($category_list[$value['category']['name']])){
                $category_list[$value['category']['name']]['category_name'] = $value['category']['name'];                
                $category_list[$value['category']['name']]['sort'] = $value['category']['sort'];                
                $category_list[$value['category']['name']]['type'] = $value['category']['type'];
                $category_list[$value['category']['name']]['data'][] = $value;
            }else{
                $category_list[$value['category']['name']] = [];
                $category_list[$value['category']['name']]['category_name'] = $value['category']['name'];
                $category_list[$value['category']['name']]['sort'] = $value['category']['sort'];
                $category_list[$value['category']['name']]['type'] = $value['category']['type'];
                $category_list[$value['category']['name']]['data'][] = $value;
        ...
        $data = array_values($category_list);
